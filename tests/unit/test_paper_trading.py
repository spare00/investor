...
from app.brokers.alpaca import BrokerError, SimulatedBroker
...
async def test_simulated_submit_and_sync(session: AsyncSession) -> None:
    ...
    om = OrderManager(session, broker=broker, controls=controls)
...
async def test_idempotent_order_not_duplicated(session: AsyncSession) -> None:
    broker = SimulatedBroker()
    om = OrderManager(session, broker=broker, controls=TradingControls())
    key = "idem-key-1"
...
async def test_validation_reject_creates_no_orders(session: AsyncSession) -> None:
    om = OrderManager(session, broker=SimulatedBroker(), controls=TradingControls())
