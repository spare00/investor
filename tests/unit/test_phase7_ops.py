...
def test_paper_automated_implies_exit_arming() -> None:
    implied = Settings(
        app_env="test",
        trading_mode="paper",
    ...
    implied_eval = GateEvaluator(settings=implied).evaluate(ReadinessGate.PAPER_AUTOMATED_CANDIDATE)
    by_implied = {c["name"]: c for c in implied_eval["checks"]}
    assert by_implied["hard_stops_armed_for_paper_auto"]["passed"] is True
    assert by_implied["force_close_armed_for_paper_auto"]["passed"] is True
    assert implied_eval["all_required_passed"] is True
...
def test_paper_automated_requires_exit_arming() -> None:
    ...
    assert by_name["intraday_paper_automated_mode"]["passed"] is False
