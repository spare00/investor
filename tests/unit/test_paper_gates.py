...
from app.market.paper_gates import (
    paper_aggressive_entries,
    paper_relaxed_data_gates,
    relax_fail_closed_reasons,
)
...
def test_paper_aggressive_entries_only_on_paper() -> None:
    assert paper_aggressive_entries(
        Settings(trading_mode=TradingMode.PAPER, live_trading_enabled=False)
    )
    assert not paper_aggressive_entries(
        Settings(
            trading_mode=TradingMode.PAPER,
            live_trading_enabled=False,
            paper_aggressive_entries=False,
        )
    )
    assert not paper_aggressive_entries(
        Settings(trading_mode=TradingMode.LIVE, live_trading_enabled=True)
