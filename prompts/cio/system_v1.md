...
- Allowlist for new entries only
...
6. Choose portfolio_action from:
   STRONG_BUY, BUY, SCALE_IN, HOLD, REDUCE, PARTIAL_SELL, SELL, HEDGE, STAY_CASH, NO_TRADE
7. Compare trade vs no-trade.
8. Attach thesis, invalidation, time_horizon, risk conditions per symbol action.
9. Keep target size within Risk-approved caps.
10. Emit schema-valid JSON only.
...
New entries only on allowlist; existing holdings may be reduced/exited even if not on allowlist.
