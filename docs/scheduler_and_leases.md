...
- When enabled, APScheduler runs:
  ...
  2. Interval job `universe_refresh` (when `UNIVERSE_MODE=dynamic` and `UNIVERSE_MANAGER_ENABLED=true`) under lease `scheduler:universe_refresh`. Poll cadence: `UNIVERSE_REFRESH_SECONDS` (default 6h). **LLM** maintains index-like membership and picks ~10 working names at most every `UNIVERSE_REFRESH_MIN_INTERVAL_DAYS` (default 7), and by default only on operator-timezone weekends (`UNIVERSE_REFRESH_WEEKEND_ONLY=true`). The weekend tick loads last CIO regime / MI themes. Weekday ticks skip the model and only run hygiene + focus rebuild. When weekend-only is off and `UNIVERSE_REFRESH_SESSION_ONLY=true`, ticks outside premarket→after-hours are skipped.
