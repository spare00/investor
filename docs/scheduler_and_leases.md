...
Job plans are created in `DailyWorkflowService.prepare` from Calendar Service session times. `intraday_eval_*` spacing follows the tightest active-watchlist horizon (scalp ≈ 2m, day ≈ 5m, …); overdue intraday jobs are coalesced to the latest. Runtime gates (`min_gap`, agent cooldowns, `MAX_INTRADAY_REANALYSES`) still skip or cap early / excess runs.
