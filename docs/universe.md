...
`TRADE_ALLOWLIST` seeds the watchlist. A curated **candidate pool** (or `UNIVERSE_CANDIDATE_POOL`) expands what the AI may add; a **liquidity screener** then filters that pool by ADV / spread / price before the manager sees it. Unknown invented tickers are still rejected.
