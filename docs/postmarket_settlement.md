...
Decision scoring is split across follow-up `postmarket_eval_*` jobs (12 decisions per 60s slice). Each chunk is **venue-scoped** (AU does not score US CIO rows, and PENDING refresh only touches that book's decisions). Already-touched ids are skipped; the first job of the session re-does leftover `PENDING` rows for that venue. There is no practical job-count cap — leftover 90-day eval work continues on later ticks instead of being dropped.
