...
When broker connection/orders are enabled, startup also runs `IntradayRecoveryService` (recon + order poll + lifecycle sync + ops alerts). See `docs/intraday_recovery.md`.
